...
//    NSLog(@"Test data: - %d",inBuffer->mAudioDataByteSize);
...
- (void)startRecordFile {
    [[XDXAudioFileHandler getInstance] startVoiceRecordByAudioUnitByAudioConverter:self.audioDecoder->mAudioConverter
                                                                   needMagicCookie:YES
    ...
    [[XDXAudioFileHandler getInstance] startVoiceRecordByAudioQueue:nil
                                                  isNeedMagicCookie:NO
                                                          audioDesc:self.audioDecoder->mDestinationFormat];
...
- (void)stopRecordFile {
    ...
    [[XDXAudioFileHandler getInstance] stopVoiceRecordByAudioQueue:nil
                                                   needMagicCookie:NO];
    NSLog(@"Audio Recorder: Stop record file.");
    
    [[XDXAudioFileHandler getInstance] stopVoiceRecordAudioConverter:self.audioDecoder->mAudioConverter
                                                     needMagicCookie:YES];
}

#pragma mark - Private
#pragma start / stop
- (BOOL)startAudioCaptureWithAudioInfo:(XDXRecorderInfoType)audioInfo formatID:(UInt32)formatID sampleRate:(Float64)sampleRate channelCount:(UInt32)channelCount durationSec:(float)durationSec bufferSize:(UInt32)bufferSize isRunning:(BOOL *)isRunning {
    if (*isRunning) {
